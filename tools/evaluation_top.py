...
parser.add_argument('-t', '--top', action='store_true', default=False, help='Switch for Top-3 accuracy.') 
...
class Evaluation:
    ...
    def print_per_class_results(self):
        ...
        for i in range(self.n_cls):
            br = self.binary_report(i)
            results_for_cls.append(br)
            auc_list.append(br['auc'])
        print('Per-class results:')
        print('Average AUC:', np.round(np.sum(auc_list)/len(auc_list), 3))
        auc_ovo = metrics.roc_auc_score(self.ground_truth, self.pred_prob, average='weighted', multi_class = 'ovo')
        print('AUC OVO:', np.round(auc_ovo, 3))
    
        auc_ovo_unweighted = metrics.roc_auc_score(self.ground_truth, self.pred_prob, average='macro', multi_class = 'ovo')
        print('AUC OVO macro:', np.round(auc_ovo_unweighted, 3))
    ...
    def print_top_3(self):
        for k in range(1,4):
            # top_score = metrics.top_k_accuracy_score(self.ground_truth, self.pred_prob, k=k, normalize=True, sample_weight=None, labels=None)
            cnt = 0
            for i in range(self.n_sample):
                idx = np.argsort(self.pred_prob[i], axis=0)[-k:]
                top_k = np.zeros(self.n_cls)
                top_k[idx] = 1
                intersec = np.sum(np.logical_and(self.ground_truth[i],top_k))
                if(intersec > 0):
                    cnt = cnt+1
            top_score = cnt/self.n_sample
            print('Top-'+str(k)+' accuracy =', np.round(top_score*100, 1), "%")
        return
...
if __name__ == "__main__":
    ...
    if args.top: evaluator.print_top_3()
